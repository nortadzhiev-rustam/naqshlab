    ...
    <div className="mx-auto max-w-3xl px-4 py-12 sm:px-6">
      {payment === "success" && (
        <div className="mb-8 flex items-center gap-4 rounded-2xl border border-emerald-200 bg-emerald-50 px-6 py-4 dark:border-emerald-800/40 dark:bg-emerald-950/30">
          ...
          <div>
            <p className="font-semibold text-emerald-800 dark:text-emerald-300">{od.paymentSuccess}</p>
            <p className="text-sm text-emerald-700/80 dark:text-emerald-400/80">
              {od.paymentSuccessDesc}
            </p>
          </div>
        </div>
      )}
      ...
      <div className="flex items-start justify-between mb-8">
        <div>
          ...
          >
            <ArrowLeft className="h-3.5 w-3.5" /> {od.title}
          </Link>
          <h1 className="text-2xl font-bold tracking-tight text-zinc-900 dark:text-zinc-100">{od.details}</h1>
        ...
        {order.items.map((item) => (
          <div key={item.id} className="flex gap-4 p-5">
            ...
            <div className="flex-1 min-w-0">
              ...
              {item.presetDesign && (
                <p className="text-xs text-zinc-400">{od.design}: {item.presetDesign.name}</p>
              )}
              {item.customizationData != null && (
                <p className="text-xs text-zinc-400">{od.customDesign}</p>
              )}
              <p className="text-xs text-zinc-400 mt-1">
                {od.qty}: {item.quantity} · ${Number(item.unitPrice).toFixed(2)} {od.each}
              </p>
            </div>
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        <div className="rounded-2xl border border-zinc-100 dark:border-zinc-800 bg-white dark:bg-zinc-900/50 p-5">
          <h3 className="text-xs font-semibold uppercase tracking-widest text-zinc-400 mb-3">{od.shippingAddress}</h3>
        ...
        <div className="rounded-2xl border border-zinc-100 dark:border-zinc-800 bg-white dark:bg-zinc-900/50 p-5">
          <h3 className="text-xs font-semibold uppercase tracking-widest text-zinc-400 mb-4">{od.summary}</h3>
