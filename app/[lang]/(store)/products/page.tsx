...
export default async function ProductsPage({
  ...
  const category = sp.category?.toUpperCase() as Category | undefined;
  const customizableOnly = sp.customizable === "true";
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-12 sm:px-6">
      {/* Header */}
      <div className="mb-10">
        <p className="text-xs font-semibold uppercase tracking-widest text-amber-500 dark:text-amber-400 mb-1">{dict.admin.productsPage.catalog}</p>
        <h1 className="text-3xl font-bold tracking-tight text-zinc-900 dark:text-zinc-100">{pd.title}</h1>
      </div>

      {/* Filters */}
      <div className="flex flex-wrap gap-2 mb-10">
        <FilterLink href={`/${locale}/products`} label={pd.all} active={!category && !customizableOnly} />
        {Object.entries(CATEGORY_LABELS).map(([key, label]) => (
          <FilterLink
            key={key}
            href={`/${locale}/products?category=${key.toLowerCase()}`}
            label={label}
            active={category === key}
          />
        ))}
        <FilterLink href={`/${locale}/products?customizable=true`} label={`✦ ${pd.customizable}`} active={customizableOnly} />
      </div>

      {products.length === 0 ? (
        <div className="text-center py-24">
          <p className="text-zinc-400 text-sm">{pd.empty}</p>
        </div>
      ) : (
                ...
                <div className="flex items-center justify-between mb-1.5">
                  ...
                  {product.isCustomizable && (
                    <span className="flex items-center gap-1 rounded-full bg-amber-50 border border-amber-200 px-2 py-0.5 text-[10px] font-semibold text-amber-600 dark:bg-amber-950/40 dark:border-amber-800/40 dark:text-amber-400">
                      <Palette className="h-2.5 w-2.5" /> {pd.customizable}
                    </span>
                  )}
                </div>
